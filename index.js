...
function parseScoresTable(err, res, done) {
  if (err) {
    ...
    let results = [];
    // let team = teams.findOne({name: 'Bordeaux PW - Green'});
...
function calculateRPI(teamGames, team) {
  ...
  const {owp: teamOWP, oowp: teamOOWP} = getTeamOWP(teamGames, team);
...
function getTeamOWP(teamGames, team) {
  ...
  return {
    owp: +(owps.reduce((total, owp) => total + owp) / owps.length).toFixed(3),
    oowp: +(oowps.reduce((total, oowp) => total + oowp) / oowps.length).toFixed(3)
  };
}
